...
import db from "@/server/db";
import { ChallengeElements, Challenges, UserProgress, UserElementCompletions } from "@/server/db/schema";
import { and, eq, sql } from "drizzle-orm";
...
export const updateUserProgress = async (userId: string) => {
    try {
        ...
        const challengeStats = await db.select({
            totalWeeks: sql<number>`count(distinct ${Challenges.id})`.as("totalWeeks"),
            completedWeeks: sql<number>`count(distinct case when challenge_completed.is_completed then ${Challenges.id} end)`.as('completedWeeks'),
            totalChallenges: sql<number>`coalesce(sum(element_counts.total_elements), 0)`.as('totalChallenges'),
            completedChallenges: sql<number>`coalesce(sum(element_counts.completed_elements), 0)`.as('completedChallenges'),
        })
        .from(Challenges)
        .innerJoin(
            db.select({
            ...
            .groupBy(ChallengeElements.challenge_id)
            .as('user_challenges'),
            eq(Challenges.id, sql`user_challenges.challenge_id`)
        )
        .leftJoin(
            db.select({
                challenge_id: ChallengeElements.challenge_id,
                is_completed: sql<boolean>`
                    count(${ChallengeElements.id}) > 0 AND 
                    count(${ChallengeElements.id}) = count(${UserElementCompletions.id})
                `.as('is_completed')
            })
            .from(ChallengeElements)
            .leftJoin(
            ...
            eq(Challenges.id, sql`challenge_completed.challenge_id`)
        )
        .leftJoin(
            db.select({
                challenge_id: ChallengeElements.challenge_id,
                total_elements: sql<number>`count(${ChallengeElements.id})`.as('total_elements'),
                completed_elements: sql<number>`count(${UserElementCompletions.id})`.as('completed_elements')
            })
            .from(ChallengeElements)
            .leftJoin(
                UserElementCompletions,
            ...
            .groupBy(ChallengeElements.challenge_id)
            .as('element_counts'),
            eq(Challenges.id, sql`element_counts.challenge_id`)
        );
...
export async function calculateCurrentStreak(userId: string): Promise<number> {
    const completedElements = await db
        .select({
            completed_at: UserElementCompletions.completed_at,
        })
        .from(UserElementCompletions)
        .where(eq(UserElementCompletions.user_id, userId))
        .orderBy(sql`${UserElementCompletions.completed_at} DESC`);
...
export async function calculateLongestStreak(userId: string): Promise<number> {
    const completedElements = await db
        ...
        .from(UserElementCompletions)
        .where(eq(UserElementCompletions.user_id, userId))
        .orderBy(UserElementCompletions.completed_at);
