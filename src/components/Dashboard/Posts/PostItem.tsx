...
const PostModal: React.FC<{
  ...
  getCurrentUserAvatar: () => JSX.Element;
  getCommentUserAvatar: (comment: Comment) => JSX.Element;
  getCommentUserDisplayName: (comment: Comment) => string;
  formatDate: (date: string) => string;
  renderPostContent: () => JSX.Element | null;
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" style={{ top: 0, left: 0, right: 0, bottom: 0, position: 'fixed' }}>
        <div className="bg-white rounded-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex relative z-10">
    ...
    try {
      ...
      const newComment: Comment = {
        ...
        author: {
          ...
          username: formData.isAnonymous ? undefined : session?.user?.username,
          image: formData.isAnonymous ? undefined : session?.user?.profilePicUrl,
